...
async function addMissingModels() {
  ...
  try {
    ...
    console.log(`📱 Adding ${missingData.models.length} missing Samsung models...`);
    
    // Prepare models for insertion
    const modelsToInsert = missingData.models.map(model => ({
...
async function runHybridIntegration() {
  console.log('🚀 Starting Hybrid Integration Solution');
  ...
  try {
    ...
    console.log('🎉 Hybrid Integration completed successfully!');
    console.log('=' .repeat(60));
    console.log('📊 Summary:');
    console.log('  ✅ Added missing Samsung models');
    ...
    console.log(`❌ Hybrid integration failed: ${error.message}`);
    return false;
  }
}

// Run the hybrid integration
runHybridIntegration(); 