...
-- Create policies for technicians (only if technicians table exists)
DO $$
BEGIN
    -- Check if technicians table exists before creating policies
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'technicians' AND table_schema = 'public') THEN
        
        -- Create policy for technicians to access their assigned bookings
        IF NOT EXISTS (
            SELECT 1 FROM pg_policies 
            WHERE policyname = 'Technicians can view their assigned bookings' 
        ...
        IF NOT EXISTS (
            SELECT 1 FROM pg_policies 
            WHERE policyname = 'Technicians can update their assigned bookings' 
            AND tablename = 'bookings'
        ) THEN
