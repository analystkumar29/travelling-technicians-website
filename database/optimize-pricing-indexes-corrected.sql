-- CORRECTED Dynamic Pricing Database Optimization Script
-- Phase 1: Remove unused indexes and add optimized composite indexes
-- SAFE TO RUN: All operations are reversible
-- FIXED: Removed constraint-backed index from DROP list

-- =============================================
-- 1. PERFORMANCE ANALYSIS BEFORE OPTIMIZATION
-- =============================================

-- Query to check current index usage (for reference)
/*
SELECT 
    schemaname,
    tablename,
...
-- =============================================
-- 2. REMOVE UNUSED INDEXES (Found in analysis)
-- =============================================

-- These indexes have 0% usage and 0 scans - safe to remove
DROP INDEX IF EXISTS public.idx_dynamic_pricing_valid;
-- NOTE: KEEPING dynamic_pricing_service_id_model_id_pricing_tier_id_key 
-- This backs a UNIQUE constraint that prevents duplicate pricing entries (essential for data integrity)
...
DROP INDEX IF EXISTS public.technicians_email_key;
DROP INDEX IF EXISTS public.warranties_warranty_code_key;
...
-- Success message
SELECT 'CORRECTED Database optimization completed successfully! ✅' as status,
       'Removed unused indexes and added optimized composite indexes (kept important constraints)' as details; 