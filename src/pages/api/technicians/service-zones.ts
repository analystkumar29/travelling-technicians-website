 ...
 * GET - Retrieve service zones for a technician or location (public, uses service role since table has no anon access)
 * POST - Add a service zone for a technician (admin only)
 * DELETE - Remove a service zone (admin only)
 */
async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { method } = req;

  // GET is public; POST/DELETE require admin auth
  if (method === 'GET') {
  ...
  return requireAdminAuth(async (authReq: NextApiRequest, authRes: NextApiResponse) => {
    switch (authReq.method) {
      case 'POST':
        return handleCreateServiceZone(authReq, authRes);
      case 'DELETE':
        return handleDeleteServiceZone(authReq, authRes);
      default:
        authRes.setHeader('Allow', ['GET', 'POST', 'DELETE']);
...
async function handleCreateServiceZone(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const { data: location, error: locError } = await serviceSupabase
