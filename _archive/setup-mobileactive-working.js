...
async function setupMobileActiveWorking() {
  console.log('🚀 Setting Up MobileActive-Based Schema (Working)');
  ...
  try {
    ...
    for (const model of samsungModels) {
      await supabase.from('device_models').insert({
        ...
        display_name: model,
        brand_id: 1,
        device_type_id: 1, // Mobile device type
        is_active: true,
        sort_order: modelId
      });
